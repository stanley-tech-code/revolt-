...
export default function TrendGuide() {
  const { db } = useCms();
  const content = db?.pages?.trendGuide || {};
      ...
      {content.heroVisible !== false && (
        ...
        <div className="relative z-10 text-center px-6 max-w-4xl mx-auto flex flex-col items-center">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold uppercase tracking-tighter text-ink mb-6 bg-canvas/90 px-6 py-2">
            {content.heroTitle || 'The Trend Guide'}
          </h1>
          <p className="text-sm md:text-base text-ink font-medium max-w-2xl bg-canvas/90 px-6 py-4 leading-relaxed">
            {content.heroDesc || "Discover the season's definitive silhouettes, color palettes, and styling directions. From conceptual layering to minimal essentials."}
          </p>
        </div>
      </section>
      )}
      ...
      <section className="py-24 px-6 md:px-12 max-w-7xl mx-auto space-y-32">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          ...
          <div className="order-1 md:order-2 space-y-6 md:pl-12">
            ...
            <h3 className="text-3xl font-bold uppercase tracking-tight text-ink">{content.trend1Title || 'Volume & Drape'}</h3>
            <p className="text-sm text-cocoa leading-relaxed whitespace-pre-wrap">
              {content.trend1Desc || "Exaggerated proportions are grounded by precision cuts. Look for drop-shoulder hoodies, pooling trousers, and outerwear that cocoons the body. The key to mastering volume is contrasting it—pair a voluminous top with a structured bottom, or vice versa."}
            </p>
            <div className="pt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          <div className="space-y-6 md:pr-12">
            ...
            <h3 className="text-3xl font-bold uppercase tracking-tight text-ink">{content.trend2Title || 'Earthy Neutrals'}</h3>
            <p className="text-sm text-cocoa leading-relaxed whitespace-pre-wrap">
              {content.trend2Desc || "The color palette is rooted in nature. Opt for shades of slate, moss, oat, and deep cocoa. Monochromatic dressing in these hues creates an instantly elevated, expensive look without requiring loud logos or aggressive styling."}
            </p>
